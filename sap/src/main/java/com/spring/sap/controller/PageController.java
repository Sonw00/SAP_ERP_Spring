...
public class PageController {
    ...
    public String addTransaction(@RequestParam("itemId") String itemId,
                                 @RequestParam("transactionType") String transactionType,
                                 @RequestParam("quantity") Integer quantity,
                                 //@RequestParam("price") Double price,
                                 @RequestParam("transactionDate") @DateTimeFormat(pattern = "yyyy-MM-dd") LocalDate transactionDate, Model model) {
        // Item을 조회
        //Item item = itemRepository.findById(itemId)
                //.orElseThrow(() -> new IllegalArgumentException("Invalid Item ID"));

    	// itemId에 해당하는 Item 엔티티를 조회
    	
    	System.out.println("TransactionForm{"+"itemId='"+itemId+'\''+", transaction Type='"+transactionType+'\''+", quantity='"+quantity+'\''+", transactionDate='"+transactionDate+'}');
    	
        Optional<Item> optionalItem = itemRepository.findById(itemId);
        if (!optionalItem.isPresent()) {
        	System.out.println("해당 ID로 아이템을 찾을 수 없습니다: " + itemId + transactionType + quantity +  transactionDate);
            model.addAttribute("error", "잘못된 제품 ID입니다.");
        ...
        } else if (transactionType.equals("판매")) {
