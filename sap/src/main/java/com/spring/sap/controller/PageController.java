...
public class PageController {
    ...
    public String addTransaction(@RequestParam("itemId") String itemName,
                                 @RequestParam("transactionType") String transactionType,
                                 @RequestParam("quantity") Integer quantity,
                                 @RequestParam("transactionDate") @DateTimeFormat(pattern = "yyyy-MM-dd") LocalDate transactionDate,
                                 Model model) {
        // itemName에 해당하는 Item 엔티티를 조회
        System.out.println("TransactionForm{" + "itemName='" + itemName + '\'' +
                           ", transactionType='" + transactionType + '\'' +
                           ", quantity='" + quantity + '\'' +
                           ", transactionDate='" + transactionDate + '}');

        // 이름이 일치하는 Item을 검색 (일치하는 항목이 하나만 있다고 가정)
        List<Item> items = itemRepository.findByNameContainingIgnoreCase(itemName);
        if (items.isEmpty()) {
            System.out.println("해당 이름으로 아이템을 찾을 수 없습니다: " + itemName);
            model.addAttribute("error", "잘못된 제품 이름입니다.");
        ...
        if (transactionType.equals("구매")) {
            if (item.getPurchasePrice() != null && quantity != null) {
                ...
                System.out.println("Purchase Price or Quantity is NULL for Item name: " + itemName);
            }
            ...
            if (item.getSellPrice() != null && quantity != null) {
                ...
                System.out.println("Sell Price or Quantity is NULL for Item name: " + itemName);
            }
        }
